...
def print_operation_table(operation, num_rows, num_columns):
    matrix = [[operation(i,j) for i in range (1, num_columns + 1)] for j in range(1, num_rows + 1)]
    print('Матрица: ')
    for i in matrix:
        print(*[f'{i:>4}' for i in i])

list1 = [int(input(f'Введите {i+1}-е число: ')) for i in range(6)]
